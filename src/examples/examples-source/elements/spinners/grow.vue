<template>
    <div class="text-center">
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="primary"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="secondary"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="success"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="danger"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="warning"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="info"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="light"></b-spinner>
        </span>
        <span class="p-2">
            <b-spinner type="grow" label="Loading..." variant="dark"></b-spinner>
        </span>
    </div>
</template>