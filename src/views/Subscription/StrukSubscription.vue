    ...
    <div class="card-body">
      <b-row>
        ...
        <b-col sm="12" style="text-align: right;" class="mt-3">
          ...
          <template v-else-if="dataPaygetDetail.detail.satuan_promo == 'percent'">
            <b-col sm="12" style="text-align: right;" class="mt-3">
              <p class="ml-2 "><strong>Potongan (%) : {{dataPaygetDetail.detail.diskon}}%</strong></p>
            </b-col>
          </template>
          <template v-else>
            <b-col sm="12" style="text-align: right;" class="mt-3">
              <p class="ml-2 "><strong>Potongan (Rp) : 0 </strong></p>
            </b-col>
          </template>
          <b-col sm="12" style="text-align: right;" class="mt-3">
            <p class="ml-2 "><strong>Total (Rp) : Rp
                {{(dataPaygetDetail.detail.amount_disc+dataPaygetDetail.paygate.biaya_admin).toFixed().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.")}}</strong>
            </p>
          </b-col>
        </b-col>
      </b-row>
